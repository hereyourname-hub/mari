...
import com.example.bodyboost.Dialogs.AddAsNeededDoseDialog;
import com.example.bodyboost.Dialogs.DoseInfoDialog;
import com.example.bodyboost.Helpers.DBHelper;
import com.example.bodyboost.Helpers.TextViewUtils;
...
public class MedicationScheduleFragment extends Fragment implements IDialogCloseListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                LocalDate finalThisDate = thisDate;
                if (meds.stream().anyMatch(m -> m.getFrequency() == 0 && !m.getStartDate().toLocalDate().isAfter(finalThisDate))) {
                    ...
                    LocalDate finalThisDate1 = thisDate;
                    plusAsNeeded.setOnClickListener(v ->
                    {
                        AddAsNeededDoseDialog asNeededDialog = null;
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                            asNeededDialog = new AddAsNeededDoseDialog(
                                    meds.stream().filter(
                                            m -> m.getFrequency() == 0 && !m.getStartDate().toLocalDate().isAfter(finalThisDate1)
    ...
    private void createSchedule() {
        ...
        String dayLabelString =
                null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            dayLabelString = dayOfWeek + " " + TimeFormatting.localDateToString(thisSunday.plusDays(dayNumber));
        }
    ...
    private RelativeLayout buildCheckbox(Medication medication, LocalDateTime time) {
        ...
        String dosageTime = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            dosageTime = TimeFormatting.formatTimeForUser(time.getHour(), time.getMinute());
        }
        ...
        ((CheckBox) thisMedication).setOnCheckedChangeListener((compoundButton, b) ->
        {
            ...
            String now = null;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                now = TimeFormatting.localDateTimeToString(LocalDateTime.now());
            }
