...
import kotlin.Pair;
import com.example.bodyboost.Fragments.MyMedicationsFragment;
...
public class MyMedications extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (String patient : patientNames) {
            ArrayList<Medication> meds = null;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                meds = db.getMedicationsForPatient(patient).stream().filter(
                        m -> m.getChild() == null
        ...
        if (allMeds.size() == 1) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                patients = (String[]) allMeds.stream().map(Pair::getFirst).toArray();
            }
            ...
            nameSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                    ...
                    Medication[] patientMeds = new Medication[0];
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                        patientMeds = (Medication[]) allMedsClone.stream().filter(
